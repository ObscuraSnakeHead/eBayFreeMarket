...
func (ts CurrentTransactionStatus) ViewCurrentTransactionStatus(lang string) ViewCurrentTransactionStatus {
	vcts := ViewCurrentTransactionStatus{
	...
	if ts.HasBeenFrozen {
		...
		vcts.CurrentFairPriceUSD = math.Floor(apis.GetHistoricCurrencyRate(ts.Type, ts.CreatedAt) * ts.CurrentAmount)
		vcts.FairPriceUSD = math.Floor(apis.GetHistoricCurrencyRate(ts.Type, ts.CreatedAt) * ts.Amount)
	}
...
func CountNumberOfTransactions(userUuid, storeUuid string) int {
	numberOfTx := 0
	if userUuid != "" {
		numberOfTx += CountCurrentTransactionStatusesForBuyer(userUuid, "", "", true)
	}
	if storeUuid != "" {
		numberOfTx += CountCurrentTransactionStatusesForStore(userUuid, "", "", true)
	}
	return numberOfTx
}

func CacheCountNumberOfTransaction(userUuid, storeUuid string) int {
	var (
		number int
		key    = "number-of-transactions-by-user-and-store-uuid-" + userUuid + "-" + storeUuid
	)

	if cNumber, _ := util.Cache1m.Get(key); cNumber == nil {
		number = CountNumberOfTransactions(userUuid, storeUuid)
		util.Cache1m.Set(key, number)
	} else {
		number = cNumber.(int)
	}
...
func RefreshShippingStatusMaterializedView() {
	database.Exec("REFRESH MATERIALIZED VIEW vm_shipping_statuses;")
}
...
func setupTransactionStatusesView() {
	...
	database.Exec("DROP MATERIALIZED VIEW IF EXISTS vm_shipping_statuses CASCADE;")
	database.Exec(`
		CREATE MATERIALIZED VIEW vm_shipping_statuses AS (
			select 
				* 
			FROM 
				v_shipping_statuses
	);`)
	database.Exec("create index on vm_shipping_statuses (transaction_uuid);")
	...
	database.Exec(`
		CREATE VIEW v_current_transaction_statuses AS (
			...
			from
				transactions t
			inner join 
				v_transaction_statuses vts on t.uuid = vts.transaction_uuid
			inner join 
				stores s on s.uuid=t.store_uuid
			inner join 
				users u2 on u2.uuid=t.buyer_uuid
			left outer join 
				vm_shipping_statuses vss on t.uuid = vss.transaction_uuid
			left outer join 
				vm_thread_counts vt on vt.parent_uuid = 'transaction-' || t.uuid
			ORDER BY
				created_at DESC
	);`)
	...
	database.Exec("DROP VIEW IF EXISTS v_current_monero_transaction_statuses CASCADE;")
	database.Exec(`
		CREATE VIEW v_current_monero_transaction_statuses AS (
			SELECT 
				vcts.*, 
				btx.amount
			FROM v_current_transaction_statuses vcts
			JOIN monero_transactions btx on vcts.uuid=btx.uuid
			WHERE vcts.type='monero'
			ORDER BY created_at DESC
	);`)
	...
	database.Exec("DROP VIEW IF EXISTS v_current_polkadot_transaction_statuses CASCADE;")
	database.Exec(`
		CREATE VIEW v_current_polkadot_transaction_statuses AS (
			SELECT 
				vcts.*, 
				etx.amount
			FROM v_current_transaction_statuses vcts
			JOIN polkadot_transactions etx on vcts.uuid=etx.uuid
			WHERE vcts.type='polkadot'
			ORDER BY created_at DESC
	);`)
	...
	database.Exec(`
		CREATE VIEW v_current_cummulative_transaction_statuses AS (
			SELECT encode(sha256(united_txs.uuid::bytea), 'hex') as uuid_hash,
united_txs.*,
CASE
	WHEN ts.id IS NOT NULL THEN TRUE
	ELSE FALSE
END AS has_been_frozen
...
FROM (
				...
				SELECT 
					...
					updated_at,
					created_at
				FROM v_current_monero_transaction_statuses
				UNION
				SELECT 
					uuid, 
					type, 
					...
					updated_at,
					created_at
				FROM v_current_polkadot_transaction_statuses
			) united_txs
			LEFT JOIN transaction_statuses ts ON united_txs.uuid = ts.transaction_uuid AND ts.Status = 'FROZEN'
			ORDER BY united_txs.created_at DESC
	);`)
}
...
func setupVendorTxStatsViews() {
	...
	database.Exec("DROP VIEW IF EXISTS v_store_monero_tx_stats CASCADE;")
	database.Exec(`
		CREATE VIEW v_store_monero_tx_stats AS (
			SELECT 
				store_uuid, 
				count(*) as tx_number, 
				sum(amount) as tx_volume 
			FROM 
				v_current_monero_transaction_statuses
			WHERE 
				current_status NOT IN ('CANCELLED', 'FAILED', 'PENDING') AND v_current_monero_transaction_statuses.created_at <= now() - interval '1 month'
			GROUP BY 
				store_uuid
	);`)
}
...
func setupItemTxStatsViews() {
	...
	database.Exec("DROP VIEW IF EXISTS v_item_monero_tx_stats CASCADE;")
	database.Exec(`
		CREATE VIEW v_item_monero_tx_stats AS (
			SELECT 
				packages.item_uuid, 
				count(*) as tx_number, 
				sum(amount) as tx_volume 
			FROM 
				v_current_monero_transaction_statuses
			JOIN
				packages on packages.uuid = v_current_monero_transaction_statuses.package_uuid
			WHERE 
				current_status NOT IN ('CANCELLED', 'FAILED', 'PENDING') AND v_current_monero_transaction_statuses.created_at <= now() - interval '1 month'
			GROUP BY 
				packages.item_uuid
