...
type StatsItem struct {
	...
	DateStr                        string    `json:"date_string"`
	Year                           int       `json:"year"`
	WeekNumber                     int       `json:"week_number"`
	NumberOfUsers                  int       `json:"number_of_users"`
	NumberOfUsersDelta             int       `json:"number_of_users_delta"`
	NumberOfVendors                int       `json:"number_of_vendors"`
	NumberOfVendorsDelta           int       `json:"number_of_vendors_delta"`
	NumberOfItems                  int       `json:"number_of_items"`
	NumberOfItemsDelta             int       `json:"number_of_items_delta"`
	NumberOfSupportMessages        int       `json:"number_of_support_messages"`
	NumberOfSupportMessagesDelta   int       `json:"number_of_support_messages_delta"`
	NumberOfDisputes               int       `json:"number_of_disputes"`
	NumberOfDisputesDelta          int       `json:"number_of_disputes_delta"`
	NumberOfBTCTransactionsCreated int       `json:"number_of_btc_tx"`
	NumberOfETHTransactionsCreated int       `json:"number_of_eth_tx"`
	NumberOfReviews                int       `json:"number_of_reviews"`
	BTCTradeAmount                 float64   `json:"btc_trade_amount"`
	ETHTradeAmount                 float64   `json:"eth_trade_amount"`
}
...
func GetMarketplaceStats(dt time.Time) []StatsItem {
	...
	btcTable := "v_current_bitcoin_transaction_statuses"
	ethTable := "v_current_ethereum_transaction_statuses"
	...
	for {
		...
		statItem := StatsItem{
			...
			// NumberOfBTCTransactionsCreated: Country
...
func CountNumberOfReleasedTransactionsForStore(userUuid string) int {
	...
	count := countTx(btcQuery) + countTx(ethQuery)
	return count
}
