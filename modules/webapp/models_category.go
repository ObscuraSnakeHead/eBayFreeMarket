...
func CacheFillCategories(packageType, countryNameEnTo, countryNameEnFrom string, cityId int) ItemCategories {
	...
	util.Cache15m.Set(key, activeCategories)
...
func findNonEmptyCategoriesByPackageType(packageType string) ItemCategories {
	cooloffPeriod := settings.GetSettings().CooloffPeriod
	query := fmt.Sprintf(`SELECT * from (
			...
			WHERE 
				ic.item_count > 0 and
				ic.hidden = false`, cooloffPeriod, cooloffPeriod)
...
func FindNonEmptyCategories(packageType, countryNameEnTo, countryNameEnFrom string, cityId int) ItemCategories {
	...
	// filter clear caterogies, remove entries with ItemCount = 0
	filteredCategories := ItemCategories{}
	for _, c := range clearCategories {
		if c.ItemCount > 0 {
