...
func (c *Context) BotCheckPOST(w web.ResponseWriter, r *web.Request) {
	...
	util.RedisClient.Set(uuid, true, 0)
...
func (c *Context) ViewVerificationPlanPurchasePOST(w web.ResponseWriter, r *web.Request) {
	...
	c.PriceBTC = c.PriceUSD * GetCurrencyRate("USD", "BTC")
	c.PriceETH = c.PriceUSD * GetCurrencyRate("USD", "ETH")
	c.PriceXMR = c.PriceUSD * GetCurrencyRate("USD", "XMR")
	...
	case "xmr":
		userWallets := c.ViewUser.User.FindUserMoneroWallets()
		if userWallets.Balance().UnlockedBalance < c.PriceXMR {
			c.Error = fmt.Sprintf("Please deposit %f XMR to your onsite wallet and wait it to unlock.", c.PriceETH)
			c.ViewVerificationPlanPurchaseGET(w, r)
			return
		}

		address, err = apis.GenerateXMRAddress("premium")
		if err != nil {
			c.Error = err.Error()
			c.ViewVerificationPlanPurchaseGET(w, r)
			return
...
func (c *Context) Login1FactorPOST(w web.ResponseWriter, r *web.Request) {
	...
	if user.TwoFactorAuthentication {
		...
		// c.Login(*user, w, r)
		c.Error = "Non 2FA logins are disabled!"
		c.LoginGET(w, r)
		return
	}
}
...
func (c *Context) ViewUserSettingsPOST(w web.ResponseWriter, r *web.Request) {
	var (
		...
		previousXMRAddress = c.ViewUser.User.Monero
		btcAddress         = r.FormValue("bitcoin")
		ethereumAddress    = r.FormValue("ethereum")
		moneroAddress      = r.FormValue("monero")
	)
	...
	if ethereumAddress != "" && !ethereumRegexp.MatchString(ethereumAddress) {
		c.Error = "Wrong Ethereum Address"
	} else {
		c.ViewUser.User.Ethereum = ethereumAddress
	}

	if moneroAddress != "" && !moneroRegexp.MatchString(moneroAddress) {
		c.Error = "Wrong Monero Address"
	} else {
		c.ViewUser.User.Monero = moneroAddress
	}
	...
	if previousXMRAddress != c.ViewUser.User.Monero {
		historyEvent := UserSettingsHistory{
			UserUuid: c.ViewUser.User.Uuid,
			Action:   "Monero address changed to " + c.ViewUser.User.Monero,
			Datetime: time.Now(),
			Type:     "monero",
		}
		if c.ViewUser.User.Monero == "" {
			historyEvent.Action = "Monero address deleted"
		}
		historyEvent.Save()
	}
...
func (c *Context) SetupPGPViaDecryptionStep2POST(w web.ResponseWriter, r *web.Request) {
	...
	if decryptedText == secretText {
		...
		http.Redirect(w, r.Request, "/settings/user?section=system", http.StatusFound)
		return
	} else {
