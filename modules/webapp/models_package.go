...
type GroupPackage struct {
	...
	PriceUSDTStr string `json:"price_usdt"`
	PriceXMRStr  string `json:"price_xmr"`
	PriceDOTStr  string `json:"price_dot"`
	// used for comparison
...
type (
	GroupedPackages      map[GroupPackage]Packages
	GroupPackagesByPrice []GroupPackage
)
...
func (avs Packages) GroupsTable() GroupedPackages {
	...
	for i := range avs {
		...
		group := GroupPackage{
			...
			PriceXMRStr:               humanize.Ftoa(av.GetPrice("XMR")),
			PriceDOTStr:               humanize.Ftoa(av.GetPrice("ASTR")),
			PriceUSDTStr:              humanize.Ftoa(av.GetPrice("USDT")),
...
func (pcks GroupedPackages) GetPackagesPage(pagenumber, pagesize int) ([]GroupPackage, int) {
	...
	numberOfPages := int(math.Ceil(float64(len(groups)) / float64(pagesize)))
