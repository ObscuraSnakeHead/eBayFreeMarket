...
var authorizedUrls = map[string]bool{
	"/":                    true,
	"/api/auth/login":      true,
...
func (c *Context) AuthMiddleware(w web.ResponseWriter, r *web.Request, next web.NextMiddlewareFunc) {
	...
	for prefix := range authorizedUrls {
		if prefix != "/" {
	...
	if userUuid != "" {
		...
		http.Redirect(w, r.Request, "/auth/login", http.StatusFound)
		return
	} else {
		next(w, r)
		return
...
func (c *Context) BotCheckMiddleware(w web.ResponseWriter, r *web.Request, next web.NextMiddlewareFunc) {
	...
	if checkUuid != "" {
		_, err := util.RedisClient.Get(checkUuid).Result()
		if err == nil {
			next(w, r)
		} else if !strings.HasPrefix(r.URL.Path, "/botcheck") && !strings.HasPrefix(r.URL.Path, "/captcha") {
			http.Redirect(w, r.Request, "/botcheck?redirect="+template.URLQueryEscaper(r.URL.String()), http.StatusFound)
		} else {
			next(w, r)
		}
	} else if !strings.HasPrefix(r.URL.Path, "/botcheck") && !strings.HasPrefix(r.URL.Path, "/captcha") {
