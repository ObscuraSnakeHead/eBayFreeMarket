...
type Transaction struct {
	...
	MoneroTransaction   *MoneroTransaction   `json:"monero_transcation,omitempty" gorm:"foreignkey:uuid"`
	PolkadotTransaction *PolkadotTransaction `json:"polkadot_transcation,omitempty" gorm:"foreignkey:uuid"`
}
...
type ViewTransactionStatus struct {
	...
	XMRPaymentResult    *apis.XMRPaymentResult    `json:"xmr_payment_result,omitempty"`
	DOTEscrowMintResult *apis.DOTEscrowMintResult `json:"dot_escrow_mint_result,omitempty"`
}
...
func (t Transaction) TransactionAmount() float64 {
	...
	case "monero":
		return t.MoneroTransaction.Amount
	case "polkadot":
		return t.PolkadotTransaction.Amount
	default:
		return float64(0.0)
	}
}

// Similar to update transaction status, but the other one is cached
func (t Transaction) UpdateTransactionFromBlockchainEscrow() error {
	switch t.Type {
	case "polkadot":
		if t.PolkadotTransaction != nil {
			t.PolkadotTransaction.UpdateTransactionStatus(t)
		}
	}
...
func (t *Transaction) SetShippingStatus(statusString string, comment, userUuid string) error {
...
func (t Transaction) UpdateTransactionStatus() error {
	...
	case "monero":
		return t.MoneroTransaction.UpdateTransactionStatus(t)
	case "polkadot":
		return t.PolkadotTransaction.UpdateTransactionStatus(t)
	}
...
func (t Transaction) Release(comment, userUuid string) error {
	...
	case "monero":
		return t.MoneroTransaction.Release(t, comment, userUuid)
	case "polkadot":
		return t.PolkadotTransaction.Release(t, comment, userUuid)
	}
...
func (t Transaction) PartialRefund(comment, userUuid string, refundPercent float64) error {
	...
	case "monero":
		return t.MoneroTransaction.PartialRefund(t, comment, userUuid, refundPercent)
...
func (t Transaction) Cancel(comment, userUuid string) error {
	...
	case "monero":
		return t.MoneroTransaction.Cancel(t, comment, userUuid)
	case "polkadot":
		return t.PolkadotTransaction.Cancel(t, comment, userUuid)
	}
...
func (t Transaction) FundFromUserWallets(user User) (PaymentReceipt, error) {
	...
	case "monero":
		moneroWallet := user.FindRecentMoneroWallet()
		balance, _ := moneroWallet.UpdateBalance(true)
		amount = t.MoneroTransaction.Amount
		if balance.Balance >= amount {
			receipt, err = moneroWallet.Send(t.Uuid, amount)
		} else {
			err = errors.New("Not enough coins in wallet")
		}
	}
...
func (t Transaction) IsCompleted() bool {
	return t.CurrentPaymentStatus() == "COMPLETED" || t.CurrentPaymentStatus() == "ESCROW"
}
...
func (t Transaction) HasBeenFrozen() bool {
	for _, status := range t.StatusSortedByTime() {
...
func CreateTransactionForPackage(
	...
	switch tp {
	case "bitcoin":
		btcTx, err := CreateBitcoinTransaction(itemPackage, buyer, tp, quantity, shippingPrice/GetCurrencyRate("BTC", "USD"))
		if err != nil {
			return nil, err
		}
		transaction.Type = "bitcoin"
		transaction.Uuid = btcTx.Uuid
		transaction.BitcoinTransaction = &btcTx

	case "monero":
		xmrTx, err := CreateMoneroTransaction(itemPackage, buyer, tp, quantity, shippingPrice/GetCurrencyRate("BTC", "USD"))
		if err != nil {
			return nil, err
		}
		transaction.Type = "monero"
		transaction.Uuid = xmrTx.Uuid
		transaction.MoneroTransaction = &xmrTx
	...
	case "polkadot":
		dotTx, err := CreatePolkadotTransaction(itemPackage, buyer, tp, quantity, shippingPrice/GetCurrencyRate("ASTR", "USD"))
		if err != nil {
			return nil, err
		}

		transaction.Type = "polkadot"
		transaction.Uuid = dotTx.Uuid
		transaction.Status = []TransactionStatus{{Time: time.Now(), Amount: dotTx.Amount, Status: "ESCROW"}}

		println("here")

		transaction.PolkadotTransaction = &dotTx
	}
...
func getTransactionQuery() *gorm.DB {
	return database.
		...
		Preload("MoneroTransaction").
		Preload("PolkadotTransaction").
		Preload("Store").
...
func FindTransactionByUuidHash(hash string) (*Transaction, error) {
	var item Transaction
	err := getTransactionQuery().Where("encode(sha256(uuid::bytea), 'hex') =?", hash).First(&item).Error
	if err != nil {
		return nil, err
	}
	return &item, err
...
func CountTransactions(status string) int {
	...
	qry := database.Table("v_current_transaction_statuses")
	if status != "" {
...
func FindCompletedTransactions() []Transaction {
	completed := findTransactionsWithCurrentStatus("COMPLETED")
	escrow := findTransactionsWithCurrentStatus("ESCROW")
	return append(completed, escrow...)
}
...
func FindPre2024CompletedAndPendingTransactions() []Transaction {
	date := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)

	completedTransactions := findTransactionsWithCurrentStatusAndTimestamp(
		"COMPLETED",
		date,
	)

	pendingTransactions := findTransactionsWithCurrentStatusAndTimestamp(
		"PENDING",
		date,
	)

	return append(completedTransactions, pendingTransactions...)
}
