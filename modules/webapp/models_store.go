...
// Ethereum returns BTC address to send released coins to
func (u Store) MoneroAddress() string {
	return u.AdminUser().MoneroAddress()
}
...
func CacheFindStoreByUserUuid(userUuid string) (*Store, error) {
	...
	if cStore, err := util.Cache15m.Get(key); err != nil && err.Error() == "Key not found." {
		store, _ = FindStoreByUserUuid(userUuid)
		util.Cache15m.Set(key, store)
	} else {
		if cStore != nil {
			store = cStore.(*Store)
		}
