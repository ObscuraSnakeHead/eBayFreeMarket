...
func (c *Context) TransactionMiddleware(w web.ResponseWriter, r *web.Request, next web.NextMiddlewareFunc) {
	...
	if transaction == nil {
		...
		transaction, _ = FindTransactionByUuidHash(r.PathParams["transaction"])
		if transaction == nil {
	...
	transaction.UpdateTransactionFromBlockchainEscrow()
	...
	if (transaction.BuyerUuid != c.ViewUser.Uuid && (c.ViewUserStore != nil && transaction.StoreUuid != c.ViewUserStore.Uuid)) && c.ViewUser.IsStaff {
		c.ViewThread = nil
	}
