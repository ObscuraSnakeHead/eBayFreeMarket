...
func (item Item) ImageBase32() string {
	content, err := os.ReadFile("./data/images/" + item.Uuid + "_200x200.jpeg")
	if err != nil {
		return ""
	}
...
type ViewItem struct {
	*Item
	...
	ViewPackages      []ViewPackage      `json:"view_packages"`
...
func (item Item) ViewItem(lang string) ViewItem {
	...
	sort.Slice(vi.ViewRatingReviews, func(i, j int) bool {
		return vi.ViewRatingReviews[i].CreatedAt.After(*vi.ViewRatingReviews[j].CreatedAt)
	})
