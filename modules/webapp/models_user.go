...
var onlineDuration, _ = time.ParseDuration(MARKETPLACE_SETTINGS.OnlineDuration)
...
type User struct {
	...
	// Tox             string `json:"tox"`
	Email           string `json:"email"` // now xmpp
	...
	UserMoneroWallets   []UserMoneroWallet   `json:"-"`
...
func (u User) FindUserPolkadotWallets() UserPolkadotWallets {
	var uw UserPolkadotWallets

	database.
		Where(&UserPolkadotWallet{UserUuid: u.Uuid}).
		Order("created_at DESC").
		Find(&uw)

	if len(uw) == 0 {
		return UserPolkadotWallets{}
	}

	return UserPolkadotWallets(uw)
}

func (u User) FindUserMoneroWallets() UserMoneroWallets {
	var uw UserMoneroWallets

	database.
		Where(&UserMoneroWallet{UserUuid: u.Uuid}).
		Order("created_at DESC").
		Find(&uw)

	if len(uw) == 0 {
		return UserMoneroWallets{}
	}

	return UserMoneroWallets(uw)
}

func (u User) FindRecentBitcoinWallet() UserBitcoinWallet {
	userWallets := u.FindUserBitcoinWallets()
	if len(userWallets) == 0 {
		return UserBitcoinWallet{}
	}
...
func (u User) FindRecentEthereumWallet() UserEthereumWallet {
	userEthereumWallets := u.FindUserEthereumWallets()
	if len(userEthereumWallets) == 0 {
		return UserEthereumWallet{}
	}
	return userEthereumWallets[0]
}

func (u User) FindRecentMoneroWallet() UserMoneroWallet {
	userMoneroWallets := u.FindUserMoneroWallets()
	if len(userMoneroWallets) == 0 {
		return UserMoneroWallet{}
	}
	return userMoneroWallets[0]
}

func (u User) FindRecentPolkadotWallet() UserPolkadotWallet {
	userPolkadotWallets := u.FindUserPolkadotWallets()
	if len(userPolkadotWallets) == 0 {
		return UserPolkadotWallet{}
	}
	return userPolkadotWallets[0]
}
...
func (s User) MoneroAddress() string {
	if s.Monero != "" {
		return s.Monero
	}

	bw := s.FindUserMoneroWallets()
	if len(bw) > 0 {
		return bw[0].PublicKey
	}
...
func CacheGetUserUuid(username string) string {
	cUuid, _ := util.Cache1h.Get(username)
	if cUuid == nil {
		user, err := FindUserByUsername(username)
		if err != nil {
			return ""
		}
		util.Cache1h.Set(username, user.Uuid)
		return user.Uuid
	}
