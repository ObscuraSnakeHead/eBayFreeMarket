...
func TaskUpdateRecentBitcoinWallets() {
	...
	util.Log.Debug("[Task] [TaskUpdateRecentBitcoinWallets], number of wallets to update:", len(recentWallets))
...
func TaskUpdateRecentEthereumWallets() {
	...
	util.Log.Debug("[Task] [TaskUpdateRecentEthereumWallets], number of wallets to update:", len(recentWallets))
	for _, w := range recentWallets {
		w.UpdateBalance(false)
	}
}
...
func StartWalletsCron() {
	gocron.Every(48).Hours().Do(TaskUpdateAllBitcoinWallets)
	gocron.Every(48).Hours().Do(TaskUpdateAllEthereumWallets)
