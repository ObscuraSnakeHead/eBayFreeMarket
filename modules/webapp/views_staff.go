...
func (c *Context) ViewStaff(w web.ResponseWriter, r *web.Request) {
	http.Redirect(w, r.Request, "/staff/users", http.StatusFound)
}
...
func (c *Context) ViewStaffCategoriesEditPOST(w web.ResponseWriter, r *web.Request) {
	...
	util.RedirectOrAPIResponse(w, r, fmt.Sprintf("/staff/item_categories/"), c, c.IsAPIRequest)
}
...
func (c *Context) ViewStaffCategoriesDelete(w web.ResponseWriter, r *web.Request) {
	...
	http.Redirect(w, r.Request, fmt.Sprintf("/staff/item_categories/"), http.StatusFound)
}
...
func (c *Context) ViewStaffListReportedMessages(w web.ResponseWriter, r *web.Request) {
	var (
		itemsPerPage     = 20
		numberOfMessages = CountReportedMessages()
	)
	...
	c.NumberOfPages = int(math.Ceil(float64(numberOfMessages) / float64(itemsPerPage)))
	// paging
	...
	c.ViewMessages = FindReportedMessages(c.Page, itemsPerPage).ViewMessages(c.ViewUser.Language)
	util.RenderTemplateOrAPIResponse(w, r, "staff/reported_messages", c, c.IsAPIRequest)
}
