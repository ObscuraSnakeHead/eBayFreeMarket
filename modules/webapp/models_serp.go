...
func (ais SerpItems) Filter(
	...
	domestic bool, mobileAppVersion bool,
) SerpItems {
...
func PriceInCurrencies(items []SerpItem) {
	for i := range items {
		items[i].Price = map[string][2]float64{
			...
			"USDT": items[i].GetPrice("USDT"),
			"XMR":  items[i].GetPrice("XMR"),
			"ASTR": items[i].GetPrice("ASTR"),
		}
	}
}
...
func cacheFindSerpItems() SerpItems {
	...
	if cItems, _ := util.Cache15m.Get(key); cItems == nil {
		items = findSerpItems()
		util.Cache15m.Set(key, items)
	} else {
...
func setupSerpItemsView() {
	...
	database.Exec(`
	CREATE UNIQUE INDEX idx_vm_available_items 
	ON vm_available_items (
		item_uuid, 
		drop_city_id,
