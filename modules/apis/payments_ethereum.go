...
func GetAmountOnETHAddress(address string) (ETHWalletBalance, error) {
	...
	if dat["balance_eth"] != nil {
		balance = dat["balance_eth"].(float64)
	}
...
func SendETH(addressFrom string, btcPayments []ETHPayment) (ETHPaymentResult, error) {
	var (
		apiEndpoint = fmt.Sprintf("%s/ethereum/wallets/%s/send_eth", APPLICATION_SETTINGS.PaymentGate, addressFrom)
