...
var (
	amberOption     = amber.Options{PrettyPrint: true, LineNumbers: false}
	amberDirOptions = amber.DirOptions{Ext: ".amber", Recursive: true}
	templates, err  = amber.CompileDir("templates", amberDirOptions, amberOption)
)
...
func RedirectOrAPIResponse(w web.ResponseWriter, r *web.Request, redirectUrl string, params interface{}, isAPIRequest bool) {
	if isAPIRequest {
...
func RenderTemplateOrAPIResponse(w web.ResponseWriter, r *web.Request, template string, params interface{}, isAPIRequest bool) {
	if isAPIRequest {
		APIResponse(w, r, params)
	} else {
		RenderTemplate(w, template, params)
	}
}
